...
void choose(int sd)
{
	while(1)
	{
		...
		if(option == 1)
		{
			attempt_userLogin(sd); 
			break;
		}
		else if(option == 2)
		{
			attemptjoint_userLogin(sd);
			break;
		}
		else if(option == 3)
		{
...
void attempt_joint_user_login(int sd)
{
	bool result;
	...
	//to the server
	write(sd,&option,sizeof(int));
	write(sd,&curr_user,sizeof(joint_user));
...
void attempt_admin_login(int sd)
{
	bool result;
	admin curr_user;
	write(1,"User ID : ",sizeof("User ID : "));
	...
	//to the server
	write(sd,&option,sizeof(int));
	write(sd,&curr_user,sizeof(admin));
...
void viewDetails(int sd)
{
	...
	{
		user curr_user1;
		read(sd,&curr_user1,sizeof(user));
		
		printf("User ID : %d\n",curr_user1.userID);
		printf("Name : %s\n",curr_user1.name);
		printf("Account Number : %d\n",curr_user1.account_no);
		printf("Available Balance : Rs.%0.2f\n",curr_user1.balance);
		printf("Status : %s\n\n",curr_user1.status);
	}
	else if(option==2)
	{
		joint_user curr_user2;
		read(sd,&curr_user2,sizeof(joint_user));
		
		printf("User ID : %d\n",curr_user2.userID);
		printf("Account Name : %s\n",curr_user2.accname);
		printf("Account Number : %d\n",curr_user2.account_no);
		printf("Available Balance : Rs.%0.2f\n",curr_user2.balance);
		printf("Status : %s\n\n",curr_user2.status);
	}
	showMenu(sd);
...
void addAccount(int sd)
{
	...
	if(type==1)
	{
		...
		write(sd,&addUser1,sizeof(user));
	}

	if(type==2)
	{
		joint_user addUser2;
		write(1,"Name of the account : ",sizeof("Name of the account : "));
		scanf(" %[^\n]",addUser2.name1);
		write(1,"Password(max 20 characters) : ",sizeof("Password(max 20 characters) : "));
...
void searchAccount(int sd){
	...
	if(type==1){
		...
		len=read(sd,&searchUser1,sizeof(user));
		if(len==0){
